...
import {Signupaction} from '../../actions/signup-action.ts'
...
const UserSignup = () => {
  ...
  if (!context) {
    throw new Error("submitHandler must be used within a UserDataProvider");
  }
  const { setUser } = context;
  const submitHandler = async (_prevState: unknown, formData: FormData) => {
    try {
      const result = await Signupaction(formData);
      console.log("Result from Signupaction:", result); // Add this line
      ...
      const [state, action, isPending] = useActionState(submitHandler, {
        success: false,
        message: "",
      });
      return (
        <div className='p-7 h-screen flex-col flex justify-between'>
          ...
          <form action={action}>
            ...
            <input disabled={isPending} type="text" placeholder="Firstname" required name='firstname' className='bg-[#eeeeee] w-1/2 rounded  px-4 py-2 border  text-lg placeholder:text-md'/>
            <input disabled={isPending} type="text" placeholder="Lastname" required name='lastname' className='bg-[#eeeeee]  w-1/2 rounded  px-4 py-2 border  text-lg placeholder:text-md'/>
            </div>
    ...
    <p className='text-center text-sm '>Already have a acoount? <Link to="/login" className='text-blue-600'>Create account</Link></p>
          </div>
          <div>
